    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果, 父亲给儿子传东西，我们用props进行传送操作，并进行接收 -->
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <!-- 对里面的每一个内容进行操作，有的内容它是有1和0的属性，我们可以使用作为布尔值的操作 -->
                <dd
                  changepirce="0"
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 
                  以前的路由跳转，从A路由跳转到B路由，这里在加入到购物车，
                  进行路由跳转之前，发请求，把你购买的产品的信息通过请求的形式通知服务器
                  服务器进行相应的存储  
                -->
...
export default {
  ...
  mounted() {
    // 什么时候获取页面的详情数据信息，一单挂载完毕就进行获取信息即可
    // 派发action获取产品详情的信息 占位的地方所使用的参数名称就叫做skuId
  ...
  methods: {
    // 产品的售卖属性值，切换为高亮,每次   对表单元素进行修改变化的操作的时候，可以使用@change事件进行操作
    changeActive(saleAttrValue, arr) {
      // 遍历全部售卖属性值 isChecked 为零，没有高亮了 先把所有的遍历属性设置为0，后面将点击的设置为1即可
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      // 点击的那个售卖属性值设置为1
    ...
    async addShopcar() {
      // alert(123)
      // 1、发请求 -- 将产品加入到数据库（通知服务器）另外一边进行了结构赋值操作，所以 对象{}里面用的就是对象的写法
      // 当前这里是派发一个action，也想服务器发请求 判断加入购物车是成功还是失败了 进行相应的操作
      // 上面的代码其实就是调用仓库中的addOrUpdateShopCart
      /* 
        要么成功要么失败
        然后在使用try...catch来进行相关操作，成功干什么，失败干什么等等
      */
      // 发请求的时候
      ...
      try {
        ...
        // 一些简单的数据skuNum，可以通过query形式给路由组件传递过去
        // 产品信息的数据【比较复杂：skuInfo】可以通过会话存储（不持久化，会话结束数据在消失）
        // 本地存储 | 会话存储 一般存储的是字符串 不允许存储字符串，可以先存储为字符串，然后使用JSON进行转化操作
